...
class Actor {
  constructor (parent, name, system, f, { shutdownAfter, whenChildCrashes } = {}) {
    ...
    this.system = system;
    this.reference = new ActorReference(this.system.sid, this.parent.reference, this.path, this.name);
    this.f = f;
  ...
  dispatch (message, sender = new Nobody(this.system.reference)) {
  ...
  query (message, timeout) {
    ...
    const tempReference = new TemporaryReference(this.system.sid);
