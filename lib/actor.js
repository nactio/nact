...
class Actor {
  constructor (parent, name, system, f, { shutdown = false } = {}) {
    this.parent = parent;
    if (!name) {
    ...
    if (shutdown) {
      ...
      this.timeout = setTimeout(this.stop.bind(this), this.shutdownPeriod);
    }
  }
  ...
  dispatch (message, sender = new Nobody(this.system)) {
    ...
    if (this.shutdownPeriod) {
      ...
      setTimeout(this.stop.bind(this), this.shutdownPeriod);
    }
    if (!this.busy) {
