...
const { logNothing } = require('./monitoring/logging-engine');
const { MessageContextLoggingEngine } = require('./monitoring/logging-actor');
...
class Actor {
  ...
  dispatch (message, sender = new Nobody(this.system.name)) {
  ...
  prepareContextForLogging(ctx) {
    ...
    if (!engine || engine === logNothing) {
      ...
      engine = new MessageContextLoggingEngine(engine, this.path.toString());
    }
