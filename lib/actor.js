...
class Actor {
  constructor (parent, name, system, f, { shutdownAfter, onCrash, initialState, initialStateFunc } = {}) {
    this.parent = parent;
    if (!name) {
  ...
  async initializeState () {
    ...
    } else if (this.initialStateFunc) {
      this.busy = true;
      try {
        this.state = await this.initialStateFunc(this.createContext(undefined));
        this.processNext();
      } catch (e) {
