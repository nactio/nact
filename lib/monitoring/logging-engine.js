...
class AbstractLoggingEngine {
  ...
  warning (message, properties, metrics) {
    this.log(
      new LogEvent(LogLevel.WARNING, 'trace', message, properties, metrics)
    );
  }
...
const logNothing = new NoopLoggingEngine();
...
class ConsoleLoggingEngine extends AbstractLoggingEngine {
  constructor (consoleProxy, formatter) {
    ...
    this.formatter =
      ...
      (logEvent =>
        ''.concat(
          '[',
          logLevelToString(logEvent.level),
          '] ',
          logEvent.category,
          ': ',
          logEvent.message
  ...
  log (logEvent) {
    ...
    const channel =
      this.channels[logEvent.level] ||
      (logEvent.level >= LogLevel.ERROR
        ? this.channels[LogLevel.ERROR]
        : undefined);
...
const logToConsole = (consoleProxy, formatter) =>
  new ConsoleLoggingEngine(consoleProxy || console, formatter);
