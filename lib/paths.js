...
class Nobody {
    ...
    tell(message,sender){
        console.log('TODO: add to dead letter queue');
    }

    static instance(system){
        if(!Nobody._val){
            Nobody._val = new Nobody(system);
        }

        return Nobody._val;    
    }

    static isNobody(path) {
        return path.type === 'nobody';
    }
...
module.exports = { TempPath, LocalPath, Nobody };